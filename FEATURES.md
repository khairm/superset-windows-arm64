...
| Auto-resume stuck chats | when a Claude chat dies on an API failure the JSONL watcher classifies it (deny-list-first: rate-limit-with-reset-time / transient-rate-limit / server-error / connection-drop / half-stop are resumable; auth/policy/bad-image/model-unavailable are notify-only) after a finality/quiescence gate, and a durable MAIN-side registry auto-sends "resume from exactly where everything was left" — fire-at-parsed-reset-time (+30s, DST-correct, fire-now-if-already-past, ≤8d sane cap) for usage limits, else a 60s→×3 backoff capped at 5 sends / 24h. Every send is gated at fire time (toggle on, still the last meaningful transcript line, host-service `commandRunning`=idle + agent-session binding match) so it can never auto-confirm a menu, land mid-turn, or run at a bare shell. Any trusted interaction with that terminal cancels (persisted tombstone survives reload/restart). Account-wide cohorts are jittered + concurrency-capped. Global toggle (default on); Codex is notify-only (no validated standalone signal). Dots untouched (separate IPC variant + toasts) | `(AUTO-RESUME)`, `autoResumeManager`, `writeInputIfIdleSession`, `useAutoResumeActivity`, `AutoResumeController` |
