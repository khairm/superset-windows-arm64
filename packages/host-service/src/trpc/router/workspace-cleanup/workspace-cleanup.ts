	...
	try {
		...
		if (input.teardownMode !== "skip" && local && project) {
			const teardown: TeardownResult = await runTeardown({
				...
				// (DISPOSE-LIMBO) the transient teardown session broadcasts its
				// confirming exit on the app's one bus.
...
async function runDestroyPhases(
	...
	if (local && project && !multiRepo) {
		...
		if (!worktreeRemoved && isMissingDirectory(project.repoPath)) {
			...
			// merely cannot read (EPERM/EACCES) is not gone, and takes the
			// "failed to open" warn-and-continue path below rather than losing
			// its worktree.
