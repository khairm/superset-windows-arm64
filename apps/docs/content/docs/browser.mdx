...
A pane runs against the browser's real, logged-in session. Confirm before asking an agent to perform consequential actions.

## Pane or standalone browser?

Use the `superset browser` commands above when a page is open in a workspace pane, such as a dev server preview or a UI flow you are watching.

Some targets are out of the panes' reach: a real Chrome window outside Superset, a host running without the desktop app attached, or work that needs an isolated or cloud browser. [Browser Use 3.0](https://docs.browser-use.com/open-source/browser-use-cli) is one optional standalone engine.
