/**
 * (MASTER-PLUS-LAUNCH) The new-workspace surface's master mode, exercised
 * through `NewWorkspaceScreen` — the component that owns both the
 * branch-vs-master rendering and every submit trigger.
 *
 * This is the same suite that used to live at
 * `DashboardNewWorkspaceForm/PromptGroup/PromptGroup.test.tsx`, repointed
 * rather than rewritten: upstream deleted `PromptGroup` (and the whole
 * `DashboardNewWorkspaceModalContent`) and made this screen the ONLY
 * new-workspace surface, so the component that renders master mode and fires
 * the master submit is now this one. The master-mode assertions that survived
 * the move are the ones the screen can still answer.
 *
 * Two assertions did NOT survive, because the UI they named never existed on
 * this screen: the modal's workspace-name / branch-name inputs and its
 * "Runs in <label>" row. Master mode is asserted here through the three
 * branch-shaped controls the screen does own — the naming-instructions gear,
 * the "Link pull request" trigger and the compare-base-branch picker — all of
 * which disappear in master mode and are all present in branch mode.
 *
 * `resolveMasterMode.test.ts` owns WHICH mode a project resolves to. This file
 * owns what the surface does once it is in one: what disappears, what stays,
 * what submit blocks on, and the order the master submit does things in.
 *
...
function installMocks(): void {
	...
	// The screen reads the window's org through this hook, and so does
	// `useWorkspaceHostUrl` underneath `useMasterWorkspaceTarget`. Stubbed
	// rather than provided, because the real one reads `CollectionsProvider`.
	mock.module("renderer/hooks/useActiveOrganizationId", () => ({
		useActiveOrganizationId: () => ORG_ID,
	}));
	...
	mock.module("renderer/lib/auth-client", () => ({
		authClient: {
			useSession: () => ({
				data: { session: { activeOrganizationId: ORG_ID } },
			}),
		},
		setAuthToken: () => {},
		getAuthToken: () => null,
	...
	// (CLOUD-SEVERANCE-P2) The submit hook reads the frozen local org id, which
	// comes off the preload bridge and throws when it is absent.
	mock.module("renderer/lib/local-identity", () => ({
		getLocalOrganizationId: () => ORG_ID,
		useActiveOrganizationId: () => ORG_ID,
		getLocalMachineId: () => MACHINE_ID,
		useLocalMachineId: () => MACHINE_ID,
	}));
	...
	// The screen's only electronTrpc call is the attachment card's
	// "reveal in Finder"; nothing here renders one.
	...
	// The project list the screen's picker and master resolver read.
	...
	// Every project counts as set up: `needsSetup` would turn submit into a
	// navigation to Settings and never reach the master path.
	mock.module("renderer/hooks/useSelectedHostProjectIds", () => ({
		useSelectedHostProjectIds: () => new Set([PROJECT_ID]),
	}));

	// No prompt-cards arm: the suggestion block is the one part of this screen
	// master mode has nothing to say about.
	...
	// below hit the REAL trigger buttons the screen renders.
	...
	mock.module("renderer/components/GitHubStarPill", () => ({
		GitHubStarPill: () => <div data-testid="github-star-pill" />,
	}));
	mock.module(
		"../DashboardNewWorkspaceForm/PromptGroup/components/GitHubIssueLinkCommand",
	...
	// Nothing here renders an attachment card (`useFileIdsForHost` is empty),
	// and its file-icon manifest is a generated resource `bun test` never builds.
	mock.module("./components/AttachmentCard", () => ({
		AttachmentCard: () => <div data-testid="attachment-card" />,
	}));
...
const { AGENT_STORAGE_KEY } = await import(
	"../DashboardNewWorkspaceForm/PromptGroup/types"
);
...
beforeEach(() => {
	...
	// The screen restores the last-used host and project from here on mount; a
	// value left behind by another test would clobber the draft below.
...
describe("NewWorkspaceScreen master mode rendering", () => {
	test("hides everything branch-shaped", () => {
		// The screen has no name/branch inputs and no "Runs in <label>" row to
		// assert on — these three are every branch-shaped control it owns.
...
describe("NewWorkspaceScreen submit gating", () => {
	...
	test("a FAILED probe falls back to the branch flow instead of spinning", async () => {
		...
		// "Checking project…" forever would strand the screen on a host whose
...
describe("NewWorkspaceScreen master submit", () => {
