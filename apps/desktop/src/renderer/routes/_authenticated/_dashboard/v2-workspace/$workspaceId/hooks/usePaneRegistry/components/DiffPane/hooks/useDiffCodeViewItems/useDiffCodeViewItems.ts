...
export function useDiffCodeViewItems({
	...
	// (NON-GIT WORKSPACE) Never run a git diff for a non-git folder — the
	...
	// (DIFF CAP) Oversized changeset: no patch is fetched at all — parsing it
	// OOMs the renderer (the vendored fork repo hit 3.6M lines). The DiffPane
	...
	const patchGroups = useMemo<PatchGroup[]>(() => {
		// (DIFF CAP) Request nothing at all above the budget.
	...
	}, [files, isTooLarge, requestedItemIds, workspaceId]);
	...
	const patchQueries = useQueries({
		queries: patchGroups.map((group) => ({
			...
			// (NON-GIT WORKSPACE) never issue a git command for a plain folder.
			enabled: isGitRepo,
	...
	return {
		...
		fileByItemId,
		requestDiff,
		hasPendingDiff: patchQueries.some((query) => query.isPending),
		hasDiffError: patchQueries.some((query) => query.isError),
		isTooLarge,
		totalChangedLines,
	};
}
