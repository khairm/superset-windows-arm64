...
function createOrgCollections(organizationId: string): OrgCollections {
	...
	// (KANBAN) Local-only board: columns + cards. Hardened like every other
	// localStorage collection — `startSync`/`gcTime: 0` so a board mutation can
	// never re-serialize the storage key from an unhydrated (empty) collection
	// and wipe the persisted board, plus quota-guarded writes. Read-healed so
	// rows persisted before a field existed normalize on read (localStorage
	// skips zod defaults).
