...
export const notificationsRouter = router({
	...
	hook: publicProcedure.input(hookInput).mutation(async ({ ctx, input }) => {
		// A subagent hook carries the terminal's roster, not its lifecycle, so it
		// bypasses the event map entirely.
		...
		if (!subagentId && !eventType) {
			// (DISPOSE-LIMBO) Every outcome is a 200, so the BODY is the only
			// place a dropped event can say why.
			warnDroppedCompanionCapture(
				input,
