...
export function loadAddons(
	...
	const rafId = requestAnimationFrame(() => {
		...
		try {
			...
			// Windows/ARM64 WebGL first-paint recovery: activation can land with an
			// empty glyph atlas and nothing scheduled to redraw, leaving a blank
			// terminal until the next keystroke. Prime the atlas and force one
			// repaint so the first frame is drawn by the renderer that just came up.
