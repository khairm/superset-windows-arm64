...
// `buildArgvCommand`/`quoteSingleShell` into @superset/shared/agent-prompt-launch,
// but that shared copy quotes `&&` into a literal argument — which would break
// custom-agent chaining — so the host router uses this fork-local pair instead.
