...
| Untagged background-agent tool completions never stomp the red | teammate/fork tool completions (ordinary Read/Bash/Edit/…) stream in attributed to the PARENT session WITHOUT an agent_id, so the SUBTOOL-RED/ASYNC-TOOL-RED checks miss them and they would Start-clear a pending AskUserQuestion red. A `<terminalId>.askq/` DIR holds one marker PER OWNER (the raising agent_id, or `_main`), set synchronously at PreToolUse:AskUserQuestion; while ANY marker exists an untagged ordinary PostToolUse maps to the red-respecting SubagentActive (and refreshes `.bgactive`, since the blocked main loop means that tool is genuine teammate activity). Per-owner so concurrent questions (main + a subagent) are independent: an owner's marker is cleared only by its own answer (PostToolUse:AskUserQuestion), its SubagentStop, or the main turn boundary (Stop/UserPromptSubmit clear only `_main`); the whole dir clears on session end / API abort / fresh SessionStart / interrupt. A main Stop while a teammate question is still open returns SubagentActive (not green) so the renderer can't clear the teammate's red. Synchronous + per-owner = no timing window and no cross-question clearing; a tool-PERMISSION red (no `.askq` marker) still clears on its approved tool's main-loop Start — no approval-clear regression | `(UNTAGGED-BG-RED)` |
