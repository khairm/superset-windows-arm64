...
| Untagged background-agent tool completions never stomp the red | teammate/fork tool completions (ordinary Read/Bash/Edit/…) stream in attributed to the PARENT session WITHOUT an agent_id, so the SUBTOOL-RED/ASYNC-TOOL-RED checks miss them and they would Start-clear a pending AskUserQuestion red. A `.askred` marker is set synchronously when the question is raised (PreToolUse:AskUserQuestion) and cleared only by its answer (PostToolUse:AskUserQuestion), a new prompt (UserPromptSubmit), or any turn/session end; while it exists an untagged ordinary PostToolUse maps to the red-respecting SubagentActive instead of Start. Marker-based so there is no background_tasks/agentbg timing window, and a tool-PERMISSION red (no `.askred`) still clears on its approved tool's main-loop Start — no approval-clear regression | `(UNTAGGED-BG-RED)` |
