...
// Re-exported for the fork's (MULTI-REPO WORKSPACE) creation flow, which
// upstream's workspaces.ts knows nothing about: multi-repo-create.ts consumes
// the same launch contract this router does. Upstream moved the definitions
// into workspace-creation/shared/dispatch-agents; the seam stays here so the
// fork module never has to track where upstream parks them next.
export { agentLaunchSchema, dispatchSugarAgents };
...
export type CloudWorkspace = CloudShapedWorkspace;
...
// (MULTI-REPO WORKSPACE) Exported: the fan-out flow builds its own container
// result and needs the same txid/idempotency primitives this router uses.
 ...
 */
export function findExistingWorkspaceByBranch(
	ctx: HostServiceContext,
	projectId: string,
	branch: string,
): CloudWorkspace | null {
...
// (MULTI-REPO WORKSPACE) Upstream moved this helper into
// workspace-creation/utils/resolve-new-branch-start-point (and generalised it to
// any base branch with an upstream). Re-export it here so fork callers that
// import it from this module (multi-repo-create) keep working.
export { resolveNewBranchStartPoint };
 ...
 *
 * (MULTI-REPO WORKSPACE) Exported for the fork's multi-repo create, which
 * resolves a start point per MEMBER repo — N in-process fetches on the event
 * loop is exactly what this replaces. */
 ...
 */
export async function registerLocalWorkspace(args: {
	ctx: HostServiceContext;
	id: string | undefined;
	projectId: string;
	name: string;
...
export const workspacesRouter = router({
	...
	create: machineOnlyProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			// dropped this with the main-workspace concept in desktop-v1.30.1.
			...
			// V1 parity: copy main-repo `.superset` into worktree if it doesn't
			// have one. Runs even when the setup terminal is skipped (imported
			// worktrees), so the config lands either way. A "local" workspace IS
			// the project's checkout, so there is nothing to copy into.
			if (
				!alreadyExists &&
				input.checkout !== "local" &&
				worktreePath !== undefined
			) {
				copyProjectSupersetConfigToWorktree(
