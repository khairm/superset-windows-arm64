...
function runStatusSnapshot(
	...
	return gitStatusRefreshLimiter.run({
		...
		run: async () => {
			...
			// (NON-GIT WORKSPACE) short-circuit before touching the worker
			// pool: a plain (non-git) workspace has no status to snapshot.
			if (!(await isGitRepo(worktreePath))) {
...
export const gitRouter = router({
	...
	getCommitFiles: queryProcedure
		...
		.query(async ({ ctx, input }) => {
			const worktreePath = resolveWorktreePath(ctx, input.workspaceId);
			// (NON-GIT WORKSPACE) a plain workspace has no commits to diff.
			if (!(await isGitRepo(worktreePath))) {
	...
	getDiff: queryProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const worktreePath = resolveWorktreePath(ctx, input.workspaceId);
			// (NON-GIT WORKSPACE) a plain folder has no diff — answer with an
			// empty pair instead of letting a git command run against it.
			if (!(await isGitRepo(worktreePath))) {
	...
	getDiffBulk: queryProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const worktreePath = resolveWorktreePath(ctx, input.workspaceId);
			// (NON-GIT WORKSPACE) a plain folder has no diffs — answer with an
			// empty pair per requested path instead of running git against it.
			if (!(await isGitRepo(worktreePath))) {
				return {
					diffs: input.paths.map((path) => {
