...
| Untagged background-agent tool completions never stomp the red | teammate/fork tool completions (ordinary Read/Bash/Edit/…) stream in attributed to the PARENT session WITHOUT an agent_id, so the SUBTOOL-RED/ASYNC-TOOL-RED checks miss them and they Start-clear a pending AskUserQuestion red. While agent-type background work is active (payload background_tasks OR a fresh agentbg hold), an untagged ordinary PostToolUse maps to the red-respecting SubagentActive — a genuine answer always arrives as UserPromptSubmit or PostToolUse:AskUserQuestion (handled first), so the red still clears on a real answer; with no agents running, the main-loop Start (permission-approval clear) is preserved | `(UNTAGGED-BG-RED)` |
