 ...
 *
 * (CLOUD-SEVERANCE-P2) Kept even though `createApiClient` below opens no
 * socket: it is a general HTTP helper, it is unit-tested on its own, and
 * deleting it would only guarantee a conflict on every future upstream change
 * to it. Nothing in this fork wires it into a live link.
 */
