...
export function useAccessibleV2Workspaces(
	...
	// Every row below is filtered against this id and the rows are served by the
	// window's own host service; upstream made it per-window for exactly that
	// reason, and here every window resolves to the same frozen organization.
		...
		(q) =>
			q
				...
				.select(({ sidebarState }) => ({
					...
					// Fork bucket fields (snooze/archive, kanban-completed, recycle-bin)
					// — the `enriched` classifier reads all of these.
					archivedAt: sidebarState.sidebarState.archivedAt,
					snoozeUntil: sidebarState.sidebarState.snoozeUntil,
					snoozeLaunchId: sidebarState.sidebarState.snoozeLaunchId,
					completedAt: sidebarState.sidebarState.completedAt,
					deletedAt: sidebarState.sidebarState.deletedAt,
				})),
		[collections],
	);
	...
	const rows = useMemo(() => {
		...
		type AccessibleRowDraft = {
			...
			// Fork bucket inputs (snooze/archive, kanban-completed, recycle-bin).
			sidebarArchivedAt: number | null;
			sidebarSnoozeUntil: number | null;
			sidebarSnoozeLaunchId: string | null;
			sidebarCompletedAt: number | null;
			sidebarDeletedAt: number | null;
			archivedAt: number | null;
			archiveReason: "merged" | "deleted" | null;
		};
				...
				return [
					{
						...
						sidebarArchivedAt: sessionSidebarState?.archivedAt ?? null,
						sidebarSnoozeUntil: sessionSidebarState?.snoozeUntil ?? null,
						sidebarSnoozeLaunchId:
							sessionSidebarState?.snoozeLaunchId ?? null,
						sidebarCompletedAt: sessionSidebarState?.completedAt ?? null,
						sidebarDeletedAt: sessionSidebarState?.deletedAt ?? null,
			...
			return [
				{
					...
					sidebarArchivedAt: sidebarState?.archivedAt ?? null,
					sidebarSnoozeUntil: sidebarState?.snoozeUntil ?? null,
					sidebarSnoozeLaunchId: sidebarState?.snoozeLaunchId ?? null,
					sidebarCompletedAt: sidebarState?.completedAt ?? null,
					...
					sidebarDeletedAt: sidebarState?.deletedAt ?? null,
					archivedAt: workspace.archivedAt ?? null,
					archiveReason: workspace.archiveReason ?? null,
				},
			];
