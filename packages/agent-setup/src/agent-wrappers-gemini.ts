...
function geminiHooksSpec(
	...
	// On Windows a bare `.sh` path is ShellExecuted by Gemini CLI and opens in
	// the user's default `.sh` editor instead of running; wrap it in Git bash
	// (see buildAgentHookCommand). Null (Windows + no Git bash) → register no
	// managed definition, while cleanEntry still drops any stale raw-.sh entry
	// written by an earlier install.
	...
	return {
		...
		desiredEntriesByEvent: Object.fromEntries(
			GEMINI_MANAGED_EVENTS.map((eventName) => [
				eventName,
				command ? [{ hooks: [{ type: "command", command }] }] : [],
			]),
		),
